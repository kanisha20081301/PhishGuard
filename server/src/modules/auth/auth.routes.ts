...
router.post('/google/quick-connect', authLimiter, async (request, response, next) => {
  try {
    ...
    const displayName = String(request.body.displayName || email.split('@')[0] || 'Google User').trim();
    ...
    response
      .cookie('phishguard_access', token, cookieOptions)
      .json({ user: { id: user.id, email: user.email, displayName: user.displayName, role: user.role } });
  } catch (error) {
    next(error);
  }
});
