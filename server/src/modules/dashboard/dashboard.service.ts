...
export async function getDashboard(userId: string) {
  ...
  const [mailbox, analyses, threatsStopped, openAlerts, recentThreats, inboxEmails] = await Promise.all([
    database.mailbox.findUnique({
      where: { id: mailboxId },
      select: { id: true, provider: true, providerUser: true, connectedAt: true, lastScannedAt: true },
    }),
    ...
    database.email.findMany({
      ...
      include: {
        analyses: {
          where: { userId },
          select: { id: true, category: true, riskLevel: true, score: true, reasons: true, modelName: true, analyzedAt: true },
        },
      },
