  ...
  async function connectWithGoogleAccount(targetEmail: string, targetName: string) {
    ...
    try {
      const result = await api<{ user: User }>('/auth/google/quick-connect', {
        method: 'POST',
        body: JSON.stringify({ email: targetEmail.trim().toLowerCase(), displayName: targetName.trim() }),
      });
  ...
  return (
    <div className="auth-screen">
      ...
      <div className="auth-visual">
        ...
        <svg className="auth-circuit-svg" viewBox="0 0 500 800" preserveAspectRatio="xMidYMid slice">
          {/* Horizontal traces */}
          <line x1="0" y1="120" x2="500" y2="120" stroke="rgba(0, 102, 255, 0.18)" strokeWidth="1.2"/>
          <line x1="0" y1="320" x2="500" y2="320" stroke="rgba(0, 102, 255, 0.18)" strokeWidth="1.2"/>
          <line x1="0" y1="520" x2="500" y2="520" stroke="rgba(0, 102, 255, 0.18)" strokeWidth="1.2"/>
          {/* Vertical traces */}
          <line x1="80" y1="0" x2="80" y2="800" stroke="rgba(0, 102, 255, 0.14)" strokeWidth="1.2"/>
          <line x1="240" y1="0" x2="240" y2="800" stroke="rgba(0, 102, 255, 0.14)" strokeWidth="1.2"/>
          <line x1="420" y1="0" x2="420" y2="800" stroke="rgba(0, 102, 255, 0.14)" strokeWidth="1.2"/>
          {/* Circuit nodes */}
          <circle cx="80" cy="120" r="6" fill="rgba(0, 102, 255, 0.1)" stroke="#0057FF" strokeWidth="2"/>
          <circle cx="240" cy="320" r="6" fill="rgba(0, 102, 255, 0.1)" stroke="#00D4FF" strokeWidth="2"/>
          <circle cx="420" cy="520" r="6" fill="rgba(0, 102, 255, 0.1)" stroke="#0057FF" strokeWidth="2"/>
          ...
          <circle cx="60" cy="600" r="28" fill="rgba(0, 102, 255, 0.05)" stroke="rgba(0, 102, 255, 0.35)" strokeWidth="1.5">
            <animate attributeName="cy" values="600;530;600" dur="16.8s" repeatCount="indefinite"/>
          </circle>
          <circle cx="380" cy="200" r="44" fill="rgba(0, 212, 255, 0.04)" stroke="rgba(0, 212, 255, 0.4)" strokeWidth="1.5">
            <animate attributeName="cy" values="200;145;200" dur="22.4s" repeatCount="indefinite"/>
          </circle>
          <circle cx="460" cy="680" r="20" fill="rgba(0, 102, 255, 0.06)" stroke="rgba(0, 102, 255, 0.3)" strokeWidth="1.2">
            <animate attributeName="cy" values="680;620;680" dur="14.4s" repeatCount="indefinite"/>
          </circle>
          <circle cx="150" cy="70" r="24" fill="rgba(0, 212, 255, 0.05)" stroke="rgba(0, 212, 255, 0.35)" strokeWidth="1.2">
        ...
        <div className="auth-visual-body">
          ...
          <p className="auth-visual-desc">
            Connect any Gmail account securely. PhishGuard reads, classifies,
            and delivers explainable virtual risk reports in real-time.
          </p>

          {/* Feature chips */}
          <div className="auth-feature-chips">
            <div className="auth-chip"><span>🔒</span> TLS Encrypted</div>
            <div className="auth-chip"><span>⚡</span> Real-time</div>
          </div>
        </div>
        ...
        <div className="auth-panel">
          ...
          <div className="auth-tabs">
            <button
              ...
              onClick={(e) => { addSplash(e); setMode('login'); setError(''); }}
            >
              Sign in
            </button>
            <button
              className={mode === 'register' ? 'active' : ''}
              onClick={(e) => { addSplash(e); setMode('register'); setError(''); }}
            >
              Create account
            </button>
          </div>

          <div className="auth-panel-label">{mode === 'login' ? 'WELCOME BACK' : 'GET STARTED'}</div>
          ...
          <button
            ...
            onClick={(e) => { addSplash(e); handleGoogleSignIn(); }}
          >
              ...
              {mode === 'register' && (
                <div className="pwd-rules-box">
                  <div className="pwd-rules-header">
                    ...
                    <span style={{ color: strengthColor }}>{['Weak','Weak','Fair','Good','Strong','Very Strong'][pwdStats.score]}</span>
                  </div>
                  <div className="pwd-strength-track">
                    <div className="pwd-strength-bar" style={{ width: strengthWidth, background: strengthColor }} />
                  </div>
        ...
        <div className="google-chooser-overlay" onClick={() => setShowGoogleChooser(false)}>
          <div className="google-chooser-modal" onClick={(e) => e.stopPropagation()}>
            ...
            <p className="google-chooser-footer">
              To continue, Google will share your name, email address, and profile picture with PhishGuard AI.
            </p>
          </div>
        </div>
      )}
...
function Dashboard({ user, onLogout }: { user: User; onLogout: () => void }) {
  ...
  const [categoryFilter, setCategoryFilter] = useState<'ALL' | 'PHISHING' | 'SPAM' | 'MARKETING' | 'SAFE'>('ALL');
  const [error, setError] = useState('');
  ...
  const countCrit = inboxList.filter((m) => m.analysis?.riskLevel?.toUpperCase() === 'CRITICAL').length;
  const countHigh = inboxList.filter((m) => m.analysis?.riskLevel?.toUpperCase() === 'HIGH').length;
  const countMed = inboxList.filter((m) => m.analysis?.riskLevel?.toUpperCase() === 'MEDIUM').length;
  const countLow = inboxList.filter((m) => ['LOW', 'SAFE'].includes(m.analysis?.riskLevel?.toUpperCase() ?? '')).length;
  ...
  const countPhish = inboxList.filter((m) => m.analysis?.category?.toUpperCase() === 'PHISHING').length;
  const countSpam = inboxList.filter((m) => m.analysis?.category?.toUpperCase() === 'SPAM').length;
  const countMkt = inboxList.filter((m) => m.analysis?.category?.toUpperCase() === 'MARKETING').length;
  ...
  return (
    <div className="react-app">
      <aside>
        ...
        <nav>
          <button
            className={tab === 'report' ? 'selected' : ''}
            onClick={() => setTab('report')}
          >
            📊 Virtual Security Report
          </button>
          <button
            className={tab === 'analyze' ? 'selected' : ''}
      ...
      <main>
        <header>
          Workspace / <b>{tab === 'report' ? 'Virtual Security Report' : tab === 'analyze' ? 'Email Inspector' : 'Alerts'}</b>
        ...
        <section className="hero">
          <div>
            ...
            <p>
              {tab === 'report' && 'Automated read-only analysis of incoming mailbox messages with explainable risk evaluation.'}
              {tab === 'analyze' && 'Test and dissect individual email headers, sender signatures, and message payloads.'}
              {tab === 'alerts' && 'Track and resolve security alerts generated from critical and high-risk phishing detections.'}
            </p>
          </div>
        </section>
              ...
              <div className="gmail-connect-card">
                <div className="gmail-info">
                  ...
                  <div className="gmail-details">
                    <h4>
                      Connected Gmail Account: <strong>{dashboard.mailbox?.providerUser ?? user.email}</strong>
              ...
              {tab === 'report' && (
                <div>
                    ...
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '12px' }}>
                      <div>
                        ...
                        <p style={{ color: '#4B5563', fontSize: '12px', margin: 0, lineHeight: '1.6' }}>
                          PhishGuard inspected all incoming Gmail messages using multi-layer heuristic and linguistic threat classification.
                        </p>
                      </div>
                      <div style={{ display: 'flex', gap: '20px', marginTop: '4px' }}>
                        {Object.entries(dashboard.categories).map(([category, percentage]) => (
                          <div key={category} style={{ textAlign: 'center' }}>
                            <div style={{ fontSize: '22px', fontWeight: 800, color: '#0A0A0F', fontFamily: 'var(--font-mono)' }}>
                              {percentage}%
                            </div>
                            <small style={{ fontSize: '10px', color: '#9CA3AF', textTransform: 'uppercase', fontFamily: 'var(--font-mono)', letterSpacing: '0.8px' }}>
                              {category}
                            </small>
                          </div>
                        ))}
                        ...
                        <div className="donut-layout">
                          ...
                          <div className="donut-legend">
                            <div className="donut-legend-item">
                              ...
                              <span className="legend-count">{countPhish} ({pctPhish}%)</span>
                            </div>
                            <div className="donut-legend-item">
                              <div className="legend-label-group">
                              ...
                              <span className="legend-count">{countSpam} ({pctSpam}%)</span>
                            </div>
                            <div className="donut-legend-item">
                              <div className="legend-label-group">
                              ...
                              <span className="legend-count">{countMkt} ({pctMkt}%)</span>
                            </div>
                            <div className="donut-legend-item">
                              <div className="legend-label-group">
                              ...
                              <span className="legend-count">{countSafe} ({pctSafe}%)</span>
                            </div>
                          </div>
                        </div>
                      </div>
                        ...
                        <div className="severity-chart">
                          <div className="severity-row">
                            <div className="severity-info">
                              <span style={{ color: '#b91c1c' }}>● Critical Risk (Credential / Financial)</span>
                              <b>{countCrit} emails ({pctCrit}%)</b>
                            </div>
                            <div className="severity-track">
                              <div className="severity-fill critical" style={{ width: `${Math.max(countCrit > 0 ? 8 : 0, pctCrit)}%` }}></div>
                            </div>
                          </div>

                          <div className="severity-row">
                            <div className="severity-info">
                              <span style={{ color: '#c2410c' }}>● High Risk (Impersonation / False Urgency)</span>
                              <b>{countHigh} emails ({pctHigh}%)</b>
                            </div>
                            <div className="severity-track">
                              <div className="severity-fill high" style={{ width: `${Math.max(countHigh > 0 ? 8 : 0, pctHigh)}%` }}></div>
                            </div>
                          </div>

                          <div className="severity-row">
                            <div className="severity-info">
                              <span style={{ color: '#a16207' }}>● Medium Risk (Suspicious Bulk / Unsolicited)</span>
                              <b>{countMed} emails ({pctMed}%)</b>
                            </div>
                            <div className="severity-track">
                              <div className="severity-fill medium" style={{ width: `${Math.max(countMed > 0 ? 8 : 0, pctMed)}%` }}></div>
                            </div>
                          </div>

                          <div className="severity-row">
                            <div className="severity-info">
                              <span style={{ color: '#15803d' }}>● Low Risk / Safe (Verified Clean Communications)</span>
                              <b>{countLow} emails ({pctLow}%)</b>
                            </div>
                            <div className="severity-track">
                              <div className="severity-fill safe" style={{ width: `${Math.max(countLow > 0 ? 8 : 0, pctLow)}%` }}></div>
                            </div>
                          </div>
                        </div>
                      </div>
                    ...
                    <div className="chart-card">
                      ...
                      <div className="vector-grid">
                        ...
                        <div className="vector-card">
                          ...
                          <span className={`vector-status ${countCrit + countHigh > 0 ? 'flagged' : 'clear'}`}>
                            {countCrit + countHigh > 0 ? 'Coercive Tactics Flagged' : 'Normal Cadence'}
                          </span>
                        </div>
                        <div className="vector-card">
                          <div className="vector-icon">🔗</div>
                          ...
                          <span className={`vector-status ${countPhish > 0 ? 'flagged' : 'clear'}`}>
                            {countPhish > 0 ? 'Suspicious Links Intercepted' : 'Zero Malicious URLs'}
                          </span>
                        </div>
                        <div className="vector-card">
                          <div className="vector-icon">🔒</div>
                  ...
                  <div className="filter-tabs">
                    ...
                    >
                      Phishing ({inboxList.filter((m) => m.analysis?.category?.toUpperCase() === 'PHISHING').length})
                    </button>
                    <button
                      className={`filter-btn ${categoryFilter === 'SPAM' ? 'active' : ''}`}
                      onClick={() => setCategoryFilter('SPAM')}
                    >
                      Spam ({inboxList.filter((m) => m.analysis?.category?.toUpperCase() === 'SPAM').length})
                    </button>
                    <button
                      className={`filter-btn ${categoryFilter === 'MARKETING' ? 'active' : ''}`}
                      onClick={() => setCategoryFilter('MARKETING')}
                    >
                      Marketing ({inboxList.filter((m) => m.analysis?.category?.toUpperCase() === 'MARKETING').length})
                    </button>
                    <button
                      className={`filter-btn ${categoryFilter === 'SAFE' ? 'active' : ''}`}
                      onClick={() => setCategoryFilter('SAFE')}
                    >
                      Safe ({inboxList.filter((m) => m.analysis?.category?.toUpperCase() === 'SAFE').length})
                    </button>
                  </div>
                  ...
                  <div className="inbox-list">
                    {filteredInbox.length === 0 ? (
                      <div className="empty-state">No email messages match the selected filter.</div>
                    ) : (
                              ...
                              {msg.analysis && (
                                <>
                                  ...
                                  <span className="category-tag">
                                    {msg.analysis.category} ({Math.round(Number(msg.analysis.score) * 100)}% Confidence)
                                  </span>
                                </>
                              )}
                            </div>
                            <span style={{ fontSize: '11px', color: '#0057FF', fontWeight: 600, fontFamily: 'var(--font-mono)' }}>
                              Inspect →
                            </span>
                          </div>
                        </div>
              ...
              {tab === 'alerts' && (
                <div className="panels">
                  <article style={{ gridColumn: '1 / -1' }}>
                    ...
                    {alerts.length === 0 ? (
                      <div className="empty-state">No open security alerts. Your mailbox is clear!</div>
                    ) : (
                      <ul>
                        {alerts.map((alert) => (
                          <li
                          ...
                          >
                            <div>
                              <strong style={{ fontSize: '13px', color: '#0A0A0F' }}>{alert.title}</strong>
                              <div style={{ fontSize: '11px', color: '#4B5563', marginTop: '3px', fontFamily: 'var(--font-mono)' }}>
                                Status: <b style={{ color: '#0057FF' }}>{alert.status}</b> · Logged at:{' '}
                ...
                <div className="modal-backdrop" onClick={() => setSelectedEmail(null)}>
                  <div className="modal-window" onClick={(e) => e.stopPropagation()}>
                    ...
                    <div style={{ marginBottom: '16px' }}>
                      <label style={{ fontSize: '10px', color: '#0057FF', fontWeight: 700, fontFamily: 'var(--font-mono)', textTransform: 'uppercase', letterSpacing: '1.5px', display: 'block', marginBottom: '8px' }}>
                      ...
                      >
                        <strong>
                          {Math.round(Number(selectedEmail.analysis?.score ?? 0.9) * 100)}% Confidence Score
                        </strong>
                        <p>Classifier Model: {selectedEmail.analysis?.modelName ?? 'phishguard-rules-v1'}</p>
                        <h4>Detection Factors &amp; Explainable Reasons:</h4>
                        <ul>
                          {selectedEmail.analysis?.reasons?.map((r) => (
                            <li key={r}>{r}</li>
                    ...
                    <div>
                      <label style={{ fontSize: '10px', color: '#0057FF', fontWeight: 700, fontFamily: 'var(--font-mono)', textTransform: 'uppercase', letterSpacing: '1.5px', display: 'block', marginBottom: '6px' }}>
                        MESSAGE CONTENT
                      </label>
                      <div
                        style={{
                ...
                <div className="scan-overlay">
                  <div className="scan-modal">
                    <div className="scan-modal-header">
                      ...
                      <p>Target Mailbox: <b>{dashboard.mailbox?.providerUser ?? user.email}</b></p>
                    </div>
                    ...
                    <div className="scan-progress-box">
                      ...
                      <div className="scan-progress-track">
                        <div className="scan-progress-bar" style={{ width: `${scanState.progress}%` }}></div>
                      </div>
                    </div>

                    <div className="scan-steps">
                      <div className={`scan-step-row ${scanState.step > 1 ? 'done' : scanState.step === 1 ? 'active' : 'waiting'}`}>
                        <div className="scan-step-icon">{scanState.step > 1 ? '✓' : '1'}</div>
                        <span className="scan-step-text">Connect to Google Mail IMAP / OAuth tunnel</span>
                        <span className="scan-step-status">{scanState.step > 1 ? 'CONNECTED' : scanState.step === 1 ? 'CONNECTING...' : 'WAITING'}</span>
                      </div>
                      <div className={`scan-step-row ${scanState.step > 2 ? 'done' : scanState.step === 2 ? 'active' : 'waiting'}`}>
                        <div className="scan-step-icon">{scanState.step > 2 ? '✓' : '2'}</div>
                        <span className="scan-step-text">Fetch inbox messages & cryptographic signatures</span>
                        <span className="scan-step-status">{scanState.step > 2 ? 'RETRIEVED' : scanState.step === 2 ? 'STREAMING...' : 'WAITING'}</span>
                      </div>
                      <div className={`scan-step-row ${scanState.step > 3 ? 'done' : scanState.step === 3 ? 'active' : 'waiting'}`}>
                        <div className="scan-step-icon">{scanState.step > 3 ? '✓' : '3'}</div>
                        <span className="scan-step-text">Validate SPF, DKIM, reverse DNS & domain age</span>
                        <span className="scan-step-status">{scanState.step > 3 ? 'INSPECTED' : scanState.step === 3 ? 'VERIFYING...' : 'WAITING'}</span>
                      </div>
                      <div className={`scan-step-row ${scanState.step > 4 ? 'done' : scanState.step === 4 ? 'active' : 'waiting'}`}>
                        <div className="scan-step-icon">{scanState.step > 4 ? '✓' : '4'}</div>
                        <span className="scan-step-text">Run AI multi-layer heuristic & NLP classifier</span>
                        <span className="scan-step-status">{scanState.step > 4 ? 'CLASSIFIED' : scanState.step === 4 ? 'ANALYZING...' : 'WAITING'}</span>
                      </div>
                      <div className={`scan-step-row ${scanState.step >= 5 ? 'done' : 'waiting'}`}>
                        <div className="scan-step-icon">{scanState.step >= 5 ? '✓' : '5'}</div>
                        <span className="scan-step-text">Synthesize Virtual Risk Graph & Executive Report</span>
                        <span className="scan-step-status">{scanState.step >= 5 ? 'READY' : 'PENDING'}</span>
                      </div>
                    </div>

                    <div className="scan-terminal">
                      {scanState.logs.map((log, i) => (
                        <div key={i} className="scan-terminal-line">{log}</div>
                      ))}
                    </div>
                  </div>
                </div>
...
function CircuitBubbleCanvas() {
  ...
  useEffect(() => {
    ...
    for (let i = 0; i < circuitPoints.length; i++) {
      ...
      if (i + cols < circuitPoints.length && Math.random() > 0.32) circuitPoints[i].connectedTo.push(i + cols);
    }
    ...
    function spawnPulse() {
      ...
      const target = fromNode.p.connectedTo[Math.floor(Math.random() * fromNode.p.connectedTo.length)];
      pulses.push({
        from: fromNode.idx,
        to: target,
        progress: 0,
    ...
    const render = () => {
      ...
      for (let i = splashes.length - 1; i >= 0; i--) {
        ...
        for (let k = 0; k < s.sparks.length; k++) {
          ...
          fgCtx.lineTo(s.x + sp.dx * (s.radius / s.maxRadius), s.y + sp.dy * (s.radius / s.maxRadius));
